...
| FR2 | Book a Service | Customers can book a laundry appointment by selecting a date (YYYY-MM-DD format) and time (HH:MM format). They choose service type (Wash, Iron, or Wash & Iron), enter item count and item types (e.g., 3 shirts, 2 pants). System calculates total price and shows it. Customer selects payment method (Cash, Credit Card, IBAN Transfer, or STC Pay). IBAN and STC Pay details are displayed for those payment options. |
...
## 7. Gaps: Features NOT Implemented

| FR | Requirement | Missing Feature |
|----|-------------|-----------------|
| - | Admin Create Appointment | Admin cannot create a new appointment for a customer (only customer can book) |
| - | Admin Modify Appointment | Admin cannot modify/change existing appointment details (date, time, service, etc.) |
